...
29. `Phase27_MainOfficeOfficerPurchaseAccess.sql` — **Phase 2 (Empty Pot Inventory / Area-specific Pot Issue) audit finding — permission grant only, run after `Phase14_RoleFoundation_AreaExtension.sql`, independent of every other phase.** No table, column, or constraint is touched. The entire purchase → stock → Area issue → production workflow (Vendors/PurchaseOrders with `ItemCategory = 'EmptyPot'` from Phase 11, `dbo.EmptyPotInventory`/its ledger from Phase 7/8, Area-to-Area issue via `InternalTransfers` `StockType = 'EmptyPot'` from Phase 8, consumption via `PotProductionRepository`) already existed; the `'Purchase.View'`/`'Purchase.Enter'` permission codes it runs on (seeded in Phase 14) had never been granted to any operational role, only to `Admin` (everything) and `Management` (`'*.View'` only) — so no day-to-day user could record a purchase or add Empty Pot stock. This script grants both codes to the existing `MainOfficeOfficer` role (already holds `MainOffice.View/Confirm` and `InternalTransfer.View/Enter` — the "Office Officer" who purchases pots and issues them to a Production Area). Idempotent (same `NOT EXISTS` guard as every Phase 14 grant); grants only, nothing is revoked.

30. `Phase28_ProductionAreaSupervisorPotProductionAccess.sql` — **Phase 3 (Mother Plant and Cutting Production) audit finding — permission grant only, run after `Phase14_RoleFoundation_AreaExtension.sql`, independent of every other phase.** No table, column, or constraint is touched. The described workflow (Mother Plant Supervisor records cutting production at their own Area, then either A: delivers it to Main Office, or B: assigns it to the appropriate Area and uses it directly for Potted Plant Production) already existed as "Model B" (`Pages/Production/CuttingStock/EnterCutting` + `GiveToMainOffice`, `Pages/Production/PotProduction/CreateFromCutting`, Phase 15/16/19). The gap: only `KiranSupervisor` (of the three cutting-producing roles seeded in Phase 14) holds `PotProduction.View`/`PotProduction.Enter`, the codes gating path B's `CreateFromCutting` page — `MotherPlantSupervisor` and `KunjirSupervisor` hold neither, so neither could complete path B themselves, only path A. This script grants both codes to both roles (mirroring `KiranSupervisor`'s existing grant). Idempotent (same `NOT EXISTS` guard as every Phase 14 grant); grants only, nothing is revoked.

31. `Phase29_CuttingDeliveryTrayCavity.sql` — **Phase 4 (Cutting Delivery to Main Office) — run after `Phase16_CuttingWorkflow_ModelB.sql`, independent of every other phase.** Adds six new, nullable columns to `dbo.InternalTransfers` — `CavityType`, `CuttingQuantityEntered`, `NumberOfTrays` (set when a Cutting transfer is created — `GiveToMainOffice`), `ActualReadyTrays`, `WastageQuantity`, `WastageReason` (set when Main Office confirms it — `ConfirmReceipt`) — plus five guarded `CHECK` constraints (closed cavity list identical to `CK_SeedSowings_CavityType`; closed wastage-reason list identical to `CK_ReadyConfirmations_WastageReason`; non-negative/positive sanity bounds on the rest). Brings the Direct Sowing/Ready Confirmation tray-by-cavity architecture (`Services/DirectSowingRules.cs`) to the existing Cutting transfer workflow instead of a freely-typed `Quantity`/`ConfirmedQuantity` pair: only cuttings that fill complete trays are ever sent (the rounding remainder stays in the source Area's Cutting Stock pool — no "remaining" field, per the explicit instruction not to add one), and Main Office's only confirmation input is Actual Ready Trays, with Actual Seedlings/Wastage always recalculated server-side. No table is created, no existing column, constraint, or workflow step (`ConfirmTransplantAsync`'s one-sided stock movement) is touched, and every pre-existing `InternalTransfers` row of any `StockType` — including already-`Transplanted`/`Rejected` Cutting rows — is unaffected: all six new columns default to `NULL`, exactly as before this script. No DB trigger enforces the tray arithmetic (application-layer only, matching every other stock workflow in this codebase); the stricter, separately-approved trigger-based hardening Direct Sowing/Ready Stock chose (`PhaseB_ReadyStockTrays.sql`) remains available later if wanted.

32. `Phase30_CuttingSowing.sql` — **Phase 5 (Cutting Sowing / Tray Production) — run after `Phase15_CuttingStock_TransferConfirmation.sql` and `Phase25_ReadyConfirmation.sql`/`PhaseB_ReadyStockTrays.sql`; independent of every other phase.** Adds a brand-new table, `dbo.CuttingSowings` (the full final shape baked in from day one — no legacy data to grandfather), mirroring `dbo.SeedSowings` but consuming `dbo.CuttingStock` instead of `dbo.SeedStock` — a deliberately separate pipeline, per "do not mix seed stock and cutting stock". Widens `CK_CuttingStockTx_Type` to add `'Sown'` (mirrors `SeedStockTransactions`'s own `'Sown'`). Widens `dbo.ReadyStock`/`dbo.ReadyConfirmations` with a nullable `CuttingSowingId` column each, a `CK_..._SourceType` CHECK requiring exactly one of `SeedSowingId`/`CuttingSowingId`, and (for `ReadyStock`) relaxes `SeedSowingId` from `NOT NULL UNIQUE` to nullable with a filtered unique index — so both Direct Sowing and Cutting Sowing feed the SAME Ready Stock/Booking/Dispatch pool, exactly like `PotProduction`'s own dual source (Phase 19). Deliberately does **not** touch the existing Direct Sowing trigger backstop (`TR_ReadyConfirmations_AssignedSupervisor`/`TR_ReadyConfirmations_TrayQuantity`/`FK_ReadyStock_SowingCavity`/`UX_ReadyConfirmations_OneConfirmedPerSowing`, all `PhaseB_ApprovalAndTrayRules.sql`/`PhaseB_ReadyStockTrays.sql`, themselves guarded `PlantsIMS2_Test`-only and "not yet approved for production") — every Cutting Sowing rule (whole trays, valid cavity, no negative/over-consumption quantities, assigned-supervisor-only approval) is enforced in `Data/CuttingSowingRepository.cs`/`Data/ReadyConfirmationRepository.cs` (`ConfirmCuttingSowingAsync`) instead, matching the dominant application-layer pattern from Phases 1-4. Every existing `SeedSowings`/`ReadyStock`/`ReadyConfirmations` row is unaffected — the new columns default to `NULL`, and `ALTER COLUMN ... NULL` never changes existing data.

33. `Phase31_PotProductionBatch.sql` — **Phase 6 (Cutting to Potted Plant Production) — run after `Phase19_CuttingToPotProduction.sql`; independent of every other phase.** Adds a brand-new header table, `dbo.PotProductionBatches` (Area/Species/Pot Size/Cutting source/Expected Ready Date/planned cutting quantity, plus running `CuttingQuantityConsumedTotal`/`QuantityProducedTotal` totals and a `Status` of `InProduction`/`Ready`/`Cancelled`), wrapping the existing `dbo.PotProduction` Cutting-sourced path (`PotProductionRepository.InsertFromCuttingStockAsync`, unchanged in its own stock-movement logic) so more than one daily production entry can be recorded against ONE plan over several days. Adds a single nullable `PotProduction.PotProductionBatchId` column + FK + index — `NULL` for every existing row (both the legacy Propagation-Batch path and the pre-existing single-event Cutting-sourced path, `PotProduction/CreateFromCutting.cshtml.cs`), which keep working completely unchanged; the column is only ever set for a new row created through a batch's own daily-entry page. No existing table, column, or row is altered. No "Color" column — per the previously approved decision (D-4) color is never a separate field, the Variety/Species name already carries it. No Responsible Person column, no second approval level — Ready confirmation reuses the existing assigned-supervisor rule (`Services/DirectSowingRules.cs` `CanApprove`, unchanged) against `SupervisorKind.ProductionArea`'s eligible pool. "An Area can never use empty pots issued to another Area" is enforced structurally: the batch's own `EmptyPotInventoryId` is fixed at creation to a pool already scoped to the source Cutting Stock's own Area, and every daily entry against the batch reuses that SAME fixed pool.

34. `Phase32_GrowingPartnerToMainOffice.sql` — **Phase 7 (Potted Plant Distribution) — run after `Phase20_GrowingPartnerToOutlet.sql`; independent of every other phase.** Ready Potted Plant Stock leaves a production/Growing Partner Area toward exactly three destinations: Main Office, Outlet, or a Direct Customer Sale. Outlet (`GrowingPartnerToOutlet`, Phase 20) and Direct Customer Sale (Booking/Dispatch, Phase 21, widened by this same phase's C# change to also accept a Main Office source) already existed; this script adds the missing symmetric, audited path to Main Office as a SIXTH `dbo.InternalTransfers` `StockType` value, `'GrowingPartnerToMainOffice'` — the mirror image of `GrowingPartnerToOutlet` (same single-step send → confirm-with-discrepancy → credit-destination shape, source must be an active Growing-Partner-linked Area, destination must be an active `AreaType='MainOffice'` Area, the opposite of `MainOfficeIssue`'s own source/destination rule). Widens only the two existing StockType-dispatch `CHECK` constraints (`CK_InternalTransfers_StockType`, `CK_InternalTransfers_SourceMatchesStockType`) — no new table, no new column, no `Status` widening (`PendingConfirmation`/`Rejected`/`Completed`/`Cancelled` already exist and are reused as-is), no change to `dbo.Area`/`AreaType`. The pre-existing, still-unrestricted generic `'PottedPlant'` Internal Transfer (Area→Area, no destination-type check) and every other existing `StockType` are completely untouched. See `Services/PottedPlantDistributionRules.cs` for the shared, unit-tested Area-type predicates this StockType's destination check and `PottedPlantBookingRepository`'s Customer Booking source check both now reuse.
