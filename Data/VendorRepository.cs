...
{
    // Simple master-data CRUD for dbo.Vendors (Phase 11). Not related to
    // dbo.VendorPurchases/VendorPurchaseRepository, which stays untouched
    // per Decision 2.
    public class VendorRepository
    {
        private readonly DatabaseHelper _dbHelper;

        public VendorRepository(DatabaseHelper dbHelper)
        {
            _dbHelper = dbHelper;
        }

        private const string BaseSelect = @"
SELECT Id, Name, ContactPerson, Phone, Email, Address, GSTIN, IsActive,
...
FROM dbo.Vendors";
        ...
        public async Task<(bool Success, string? Message, int Id)> InsertAsync(Vendor entry)
        {
...
INSERT INTO dbo.Vendors (Name, ContactPerson, Phone, Email, Address, GSTIN, IsActive, CreatedDate, CreatedBy)
VALUES (@Name, @ContactPerson, @Phone, @Email, @Address, @GSTIN, @IsActive, SYSUTCDATETIME(), @CreatedBy);
SELECT CAST(SCOPE_IDENTITY() AS INT);";
                ...
                cmd.Parameters.AddWithValue("@GSTIN", (object?)entry.GSTIN ?? DBNull.Value);
        ...
        public async Task<(bool Success, string? Message)> UpdateAsync(Vendor entry)
        {
            if (string.IsNullOrWhiteSpace(entry.Name))
...
SET Name = @Name, ContactPerson = @ContactPerson, Phone = @Phone, Email = @Email,
    Address = @Address, GSTIN = @GSTIN, IsActive = @IsActive,
...
WHERE Id = @Id";
                ...
                return rows > 0 ? (true, null) : (false, "Vendor not found.");
            }
            catch (Exception ex)
            {
        ...
        private static Vendor Map(SqlDataReader reader)
        {
            return new Vendor
            {
                ...
                GSTIN = reader.IsDBNull(reader.GetOrdinal("GSTIN")) ? null : reader.GetString(reader.GetOrdinal("GSTIN")),
