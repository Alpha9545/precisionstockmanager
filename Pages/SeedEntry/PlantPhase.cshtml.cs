...
{
    [Authorize]
    public class PlantPhaseModel : PageModel
    {
        private readonly DatabaseHelper _dbHelper;
        public List<SeedEntries> SeedEntries { get; set; }

        public PlantPhaseModel(DatabaseHelper dbHelper)
        {
            _dbHelper = dbHelper;
        }
                ...
                using (var reader = await cmd.ExecuteReaderAsync())
                {
                    while (await reader.ReadAsync())
                    {
                        records.Add(new SeedEntries
                        {
                            ...
                            //TraysAlive = reader.GetInt16(7),
                            //HardeningAlive = reader.GetInt16(8),
                            //AliveCount = reader.GetInt16(9),
        ...
        public async Task<IActionResult> OnPostAddToInventoryAsync(int SeedEntryId, int AliveCount, string NextStage)
        {
            try
            {
                       ...
                       WHERE Id = @Id",
                        ...
                        if (nextStage == "Inventory")
                        {
                            var cmdInventory = new SqlCommand(@"
                                INSERT INTO Inventory (PlantId, SpeciesId, PolyhouseId, SeedEntryId, Quantity, LastUpdated)
                                OUTPUT INSERTED.Id 
                                SELECT s.PlantId, s.SpeciesId, s.PolyhouseId, s.Id, @AliveCount, GETDATE()
                                FROM SeedEntries s
